 ...
 **/
@ServerEndpoint(value = "/accept")
@Component
@Slf4j
@CrossOrigin(origins = "*")
public class AcceptWebSocketService {
    ...
    public static void setApplicationContext(ApplicationContext applicationContext){
        AcceptWebSocketService.applicationContext = applicationContext;
    }
    ...
    public void onMessage(Session session, String message) {
        ...
        try {
            session.getBasicRemote().sendText("我接收到了消息，调度信息已经接收");
        } catch (Exception e) {
            log.error("保存失败");
        }
    ...
    private void initSpringBean() {
        ...
        synchronized (AcceptWebSocketService.class) {
            if (taskService == null) {
